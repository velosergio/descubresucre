  ...
  **Roles (RBAC en base de datos):**
  ...
  | Rol      | Acceso |
  | -------- | ------ |
  | `admin`  | Tablero, **Personalizar**, usuarios, roles y **Configuración** del chatbot (sidebar completo) |
  | `editor` | Tablero (`/admin`) con sesión de staff; las rutas de gestión avanzada requieren rol `admin` |

  Las rutas `/admin/personalizar/*`, `/admin/users`, `/admin/roles` y `/admin/configuracion` validan **`requireAdminSession`** (solo administradores).
  ...
  **Rutas del dashboard:**
  ...
  | `/admin/personalizar`  | Hub de personalización visual del sitio (**admin**) |
  ...
  **API de medios local (galería):**

  | Método y ruta                    | Uso |
  | -------------------------------- | --- |
  | `GET /api/media?src=/uploads/...`| Servir archivos de `public/uploads` con validación de ruta y cacheo fuerte |
  ...
  **Contrato sugerido del callback (JSON):** `{ "jobId": "…", "reply": "markdown o texto", "secret": "…" }` (alternativa: `output` en lugar de `reply`). Para errores: `{ "jobId": "…", "error": "…", "secret": "…" }`.
  ...
  - `npm run test:e2e:ui`: modo UI de Playwright para depuración local.
