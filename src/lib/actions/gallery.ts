  ...
  try {
    ...
    if (
      isGalleryUrlUsedBySucreNatural(
        {
          cardImageUrls: destCards.map((d) => d.cardImageUrl),
          galleryItemUrls: galleryItems.map((g) => g.publicUrl),
          hubCoverUrls: hubCovers.map((h) => h.coverImageUrl),
          speciesImageUrls: speciesImgs.map((s) => s.imageUrl),
          experienceImageUrls: expImgs.map((e) => e.imageUrl),
        },
        asset.publicUrl,
      )
    ) {
