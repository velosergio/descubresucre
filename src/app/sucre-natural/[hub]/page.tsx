...
import { getBiodiversityHub, getExperiencesHub, getHubPage } from "@/lib/get-sucre-natural-public";
...
export default async function SucreNaturalHubPage({ params }: Props) {
  ...
  const [species, experiences] =
    page.hub.id === "biodiversidad"
      ? [await getBiodiversityHub(), []]
      : page.hub.id === "experiencias"
        ? [[], await getExperiencesHub()]
        : [[], []];
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-10 px-4 py-10" data-hub={page.hub.id}>
      ...
      {page.hub.id === "biodiversidad" && species.length > 0 ? (
      ...
      {page.hub.id === "experiencias" && experiences.length > 0 ? (
      ...
      page.catalogNote ? (
        <p className="font-body text-[hsl(var(--sn-ink)/0.7)]">{page.catalogNote}</p>
      ) : null}
