...
const RechartsModuleContext = React.createContext<RechartsMod | null>(null);
  ...
  if (!mod) {
    return (
      <div
        ref={ref}
        className={cn(
          "flex aspect-video min-h-[12rem] animate-pulse items-center justify-center rounded-md bg-muted/40 text-xs",
          className,
        )}
        aria-busy
    ...
    <ChartContext.Provider value={{ config }}>
      <RechartsModuleContext.Provider value={mod}>
        ...
        >
          ...
          <mod.ResponsiveContainer>{children}</mod.ResponsiveContainer>
        </div>
      </RechartsModuleContext.Provider>
...
type ChartTooltipProps = React.ComponentProps<RechartsMod["Tooltip"]>;

function ChartTooltip(props: ChartTooltipProps) {
  const mod = React.useContext(RechartsModuleContext);
  if (!mod) return null;
  return <mod.Tooltip {...props} />;
}
...
type ChartLegendProps = React.ComponentProps<RechartsMod["Legend"]>;

function ChartLegend(props: ChartLegendProps) {
  const mod = React.useContext(RechartsModuleContext);
  if (!mod) return null;
  return <mod.Legend {...props} />;
}
